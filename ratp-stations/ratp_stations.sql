...
-- Define all exterior train stations which have only 1 line
UPDATE ratp_stations SET train_outside = TRUE WHERE "name" IN ('','');

-- Define all exterior train stations which have several lines
UPDATE ratp_stations SET train_outside = TRUE WHERE "coordinates" IN ('48.8533891123,2.36916460221','');


-- Add lines for all train stations which have only 1 line
...
-- Add lines for all train stations which have several lines
